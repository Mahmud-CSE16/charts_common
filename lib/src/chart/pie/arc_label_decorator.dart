...
import 'dart:math' show cos, min, sin, pi, Point, Rectangle, sqrt2, sqrt;
...
class ArcLabelDecorator<D> extends ArcRendererDecorator<D> {
  ...
  void decorate(ArcRendererElementList<D> arcElements, ChartCanvas canvas,
    ...
    for (var element in arcElements.arcs) {
      ...
      final color = element.series.colorFn(datumIndex);
  ...
  List _drawOutsideLabel(
    ...
    final offsetY = 13 * sin(centerAngle);
    canvas.drawText(labelElement, labelX, labelY - 8 + offsetY.toInt());
  ...
  double _drawLeaderLine(
    ...
    final offsetX = 13.0 * cos(centerAngle);
    final offsetY = 13.0 * sin(centerAngle);
    final lineLength = (labelLeftOfChart ? -1 : 1) *
        (83.0 - leaderLineStyleSpec.length - sqrt(offsetX * offsetX));
    final leaderLineTailPoint = new Point<double>(
        labelPoint.x + tailX + lineLength + offsetX, labelPoint.y + offsetY);
    final offsetLabelPoint =
        new Point<double>(labelPoint.x + offsetX, labelPoint.y + offsetY);
    final centerRadius = radius - leaderLineStyleSpec.length / 2;
    final leaderLineStartPoint = new Point<double>(
        arcCenterPoint.x + offsetX + centerRadius * cos(centerAngle),
        arcCenterPoint.y + offsetY + centerRadius * sin(centerAngle));
    ...
    canvas.drawLine(
        ...
        stroke: color ?? leaderLineStyleSpec.color,
        strokeWidthPx: leaderLineStyleSpec.thickness);
    canvas.drawPoint(
        point: leaderLineStartPoint,
        radius: 3,
        fill: color ?? leaderLineStyleSpec.color);
